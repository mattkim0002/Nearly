...
export default function ListsPage({ user, onSignOut }: ListsPageProps) {
  ...
  useEffect(() => {
    ...
    const loadedFavorites = JSON.parse(localStorage.getItem('bluedot_favorites') || '[]');
    console.log('Loaded favorites:', loadedFavorites);
    setFavorites(loadedFavorites);
    
    // Load saved lists
    const savedLists = JSON.parse(localStorage.getItem('bluedot_lists') || '[]');
  ...
  const handleCreateList = () => {
    if (newListName.trim()) {
      ...
      localStorage.setItem('bluedot_lists', JSON.stringify(updatedLists));
                ...
                {filteredFavorites.map((item: any) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="absolute top-3 right-3 w-8 h-8 bg-red-50 border border-red-200 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition hover:bg-red-100"
