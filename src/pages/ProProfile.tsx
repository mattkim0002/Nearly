    ...
    <div className="min-h-screen bg-gradient-to-b from-sky-50 to-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="bg-white rounded-3xl shadow-xl border-2 border-slate-200 p-8 mb-8 relative">
          ...
          <button
            ...
            className={`absolute top-4 right-4 w-14 h-14 rounded-full flex items-center justify-center hover:scale-110 transition shadow-lg z-50 ${
              isFavorited 
                ...
                : 'bg-white hover:bg-sky-50 border-2 border-slate-300 hover:border-sky-500'
            }`}
          ...
          <div className="flex flex-col md:flex-row gap-8">
            <div className="flex-shrink-0">
              <div className="w-32 h-32 rounded-2xl bg-gradient-to-br from-sky-600 to-blue-600 flex items-center justify-center text-white font-bold text-4xl">
            ...
            <div className="flex-1">
              ...
              <button onClick={handleContact} className="w-full md:w-auto px-8 py-3 rounded-xl bg-sky-600 text-white font-semibold hover:bg-sky-700 transition shadow-lg">
        ...
        <div className="flex gap-2 mb-8 bg-white rounded-xl p-1 border-2 border-slate-200 w-fit">
          <button onClick={() => setSelectedTab('overview')} className={`px-6 py-2 rounded-lg font-medium transition ${selectedTab === 'overview' ? 'bg-sky-600 text-white' : 'text-slate-600 hover:text-slate-900'}`}>
            Overview
          </button>
          <button onClick={() => setSelectedTab('portfolio')} className={`px-6 py-2 rounded-lg font-medium transition ${selectedTab === 'portfolio' ? 'bg-sky-600 text-white' : 'text-slate-600 hover:text-slate-900'}`}>
            Portfolio ({provider.portfolio?.length || 0})
          </button>
          <button onClick={() => setSelectedTab('reviews')} className={`px-6 py-2 rounded-lg font-medium transition ${selectedTab === 'reviews' ? 'bg-sky-600 text-white' : 'text-slate-600 hover:text-slate-900'}`}>
                ...
                <div className="bg-white rounded-2xl shadow-md border-2 border-slate-200 p-8">
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {provider.skills?.map((skill: string, index: number) => (
                      <span key={index} className="px-4 py-2 bg-sky-100 text-sky-700 rounded-full text-sm font-medium">{skill}</span>
                    ))}
                  </div>
                  ...
                  <div className="bg-white rounded-2xl shadow-md border-2 border-slate-200 p-8">
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                      {provider.packages?.map((pkg: any, index: number) => (
                        <div key={index} className={`relative rounded-2xl border-2 p-6 cursor-pointer transition ${pkg.popular ? 'border-sky-500 bg-sky-50' : 'border-slate-200 hover:border-sky-300'}`}>
                          {pkg.popular && (
                            <span className="absolute -top-3 left-1/2 transform -translate-x-1/2 px-3 py-1 bg-sky-600 text-white text-xs font-bold rounded-full">
                              MOST POPULAR
                            </span>
                          )}
                          <h3 className="font-bold text-xl text-slate-900 mb-2">{pkg.name}</h3>
                          <p className="text-3xl font-bold text-sky-600 mb-1">{pkg.price}</p>
                          ...
                          <button onClick={handleContact} className={`w-full py-2 rounded-lg font-semibold transition ${pkg.popular ? 'bg-sky-600 text-white hover:bg-sky-700' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>
                            Select Package
                          </button>
                        </div>
                      ))}
                ...
                {provider.portfolio && provider.portfolio.length > 0 ? (
                  <div className="grid md:grid-cols-2 gap-6">
                    {provider.portfolio?.map((item: any) => (
                      <div key={item.id} className="group border-2 border-slate-200 rounded-2xl overflow-hidden hover:border-sky-400 transition cursor-pointer">
                        ...
                        <div className="p-4">
                          <div className="flex items-start justify-between mb-2">
                            ...
                            <span className="text-sm font-semibold text-sky-600">{item.price}</span>
                          </div>
                      ...
                      <div key={review.id} className="border-b border-slate-200 last:border-0 pb-6 last:pb-0">
                        <div className="flex items-start gap-4 mb-3">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-sm font-semibold text-sky-600 mb-2">{review.project}</p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-8">
            ...
            <div className="mb-6">
              ...
              <textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Hi! I'm interested in your services..." rows={6} className="w-full px-4 py-3 rounded-xl border-2 border-slate-300 focus:border-sky-500 focus:outline-none resize-none" />
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleSendMessage} className="flex-1 py-3 rounded-xl bg-sky-600 text-white font-semibold hover:bg-sky-700 transition">Send Message</button>
            </div>
          </div>
        </div>
      )}
