            ...
            {[1, 2, 3].map((num) => (
              <div key={num} className="flex items-center">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${
                  step >= num ? 'bg-orange-500 text-white' : 'bg-slate-200 text-slate-600'
                }`}>
                  {step > num ? '✓' : num}
                </div>
                {num < 3 && (
                  <div className={`w-16 h-1 ${step > num ? 'bg-orange-500' : 'bg-slate-200'}`}></div>
                )}
              </div>
            ))}
          </div>
                  ...
                  {skills.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {skills.map((skill, index) => (
                        <div key={index} className="flex items-center gap-2 px-3 py-2 bg-orange-50 text-orange-600 rounded-full">
                          <span className="text-sm font-semibold">{skill}</span>
                          <button
                            onClick={() => handleRemoveSkill(index)}
                            className="text-orange-500 hover:text-sky-800"
                          >
                            ×
                          </button>
                        </div>
            ...
            <>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="50"
                      className="w-full pl-8 pr-4 py-3 rounded-xl border-2 border-slate-300 focus:border-orange-600 focus:outline-none"
                    />
                  </div>
                  <p className="text-xs text-slate-500 mt-1">
              ...
              <div className="flex gap-3 mt-8">
                ...
                <button
                  ...
                  disabled={loading}
                  className="flex-1 py-3 rounded-xl bg-orange-500 text-white font-semibold hover:bg-orange-600 transition disabled:opacity-50"
                >
